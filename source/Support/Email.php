...
class Email
{
     ...
     * @return Email
     */
    public function bootstrap(string $subject, string $body, string $recipient, string $recipientName, string $bcc = null): Email
    {
        $this->data->subject = $subject;
        $this->data->body = $body;
        $this->data->recipient_email = $recipient;
        $this->data->recipient_name = $recipientName;
        $this->data->bcc = ($bcc ? $bcc : null);
        return $this;
    }
     ...
     * @return Email
     */
    public function attachPDF(string $file, string $fileName): Email
    {
        $this->data->attachPdf = $file;
        $this->data->attachPdfName = $fileName;
        return $this;
    }


    /**
     * Add bcc email - email@test.com;email2@test.com
     * @param string $emails
     * @return Email
     */
    public function addBcc(string $emails): Email
    {
        $this->data->addBCC($emails);
        return $this;
    }
     ...
     * @return bool
     */
    public function send(string $from = CONF_MAIL_SENDER['address'], string $fromName = CONF_MAIL_SENDER["name"], $strAttachment = null, $nameAttach = null): bool
    {
        if (empty($this->data)) {
            $this->message->error("Error sending, please check the values");
            return false;
        }

        if (!is_email($this->data->recipient_email)) {
            $this->message->warning("Mail recipient is not valid");
            return false;
        }

        if (!is_email($from)) {
            $this->message->warning("Sender email is not valid");
            return false;
        }
        ...
        try {
            ...
            if($this->data->bcc){
                $emailsTemp = explode(";",$this->data->bcc);
                foreach ($emailsTemp as $email){
                    $this->mail->addBCC($email,'UCB Neurology');
                }
            }

            if($strAttachment){
                if ($nameAttach){
                    $nameAttach  = $nameAttach.".ics";
                }
                else{
                    $nameAttach = 'event.ics';
                }
                $this->mail->addStringAttachment($strAttachment, $nameAttach,'base64','text/calendar');
            ...
            if (!empty($this->data->attachPdf)){
                $this->mail->addStringAttachment($this->data->attachPdf, $this->data->attachPdfName, 'base64','application/pdf');
            }
...
//                var_dump($send);
    ...
    /**
     * Save a email copy on the server usage: save_mail() goes to inbox; save_mail('Sent') goes to the Sent folder
     * @param null $folderPath
     */
    public function save_mail($folderPath = null)
    {
        $path = "INBOX" . (isset($folderPath) && !is_null($folderPath) ? ".".$folderPath : "");
        //Formating: {imap.server.com:993/imap/ssl}/Folder(INBOX, Sent)
        $mailBox = "{".$this->mail->Host.":".CONF_MAIL_IMAP_PORT."/imap/".$this->mail->SMTPSecure."}".$path;
        $imapStream = imap_open($mailBox , $this->mail->Username, $this->mail->Password);
        imap_append($imapStream, $mailBox, $this->mail->getSentMIMEMessage());
