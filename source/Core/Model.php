...
//        self::$protected = array_merge($protected, ['created_at', 'updated_at']);
        ...
        $this->message = 'new message class';
    }
...
//            var_dump("INSERT INTO " . static::$entity . " ({$columns}) VALUES ({$values})");
...
//            $this->message->warning("Fill in all fields to continue");
        ...
        if (!empty($this->id)) {
            ...
            if ($this->fail()) {
//                $this->message->error("Error updating, check the data !!");
                echo "Error updating, check the data !!";
                return false;
            }
        }
        ...
        if (empty($this->id)) {
            ...
            if ($this->fail()) {
//                $this->message->error("Error when registering, check the data !!");
                echo "Error when registering, check the data !!";
                return false;
            }
        }
    ...
    /**
     * Returns TRUE if the registration or update is performed or FALSE if not. To verify
     * errors execute a getError ();
    ...
    /**
     * return an array associated with the message and the kind of error!
     * @return ARRAY [message, WS_ACCEPT ]  = Array associated with the error
     */
